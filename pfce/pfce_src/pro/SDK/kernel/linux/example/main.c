...
extern int peach_started;
extern char* peach_data;
extern size_t peach_data_size;
...
static int scullv_read_mem_proc_show(struct seq_file *m, void *v)
{
	...
	// vv PEACH
	if(peach_started && peach_data && peach_data_size > 0)
	{
	  peach_data[peach_data_size-1] = 0;
	  seq_printf(m, "%s", (char*)peach_data);
	}
	// ^^ PEACH
